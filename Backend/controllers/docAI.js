...
const clientlan = new language.LanguageServiceClient({
    keyFilename:
        "D:/Codes/Web_Development/SIH1633/Backend/natural-pipe-435404-f3-3e7d64d566f7.json", // Update with the path to your JSON credentials file
});
...
const docAI = (req, res) => {
    upload(req, res, async (err) => {
        ...
        try {
            ...
            const matchedYear = fuse.search(
                user.gmonth + " " + user.gyear.toString()
            );
            ...
            if (
                ...
                matchedClg.length > 0 &&
                matchedDegree.length > 0 &&
                matchedYear.length > 0 &&
                matchedRoll.length > 0
            ) {
